...
import { registerDeviceToken, removeDeviceToken } from '@/api/account';
...
/**
 * Attempts to retrieve the device push token and deregister it from the backend.
 * Called during logout.
 */
export async function tryRemovePushToken(): Promise<void> {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-explicit-any
    const Notifications = require('expo-notifications') as any;

    const { status } = await Notifications.getPermissionsAsync();
    // Cannot fetch token if permission not granted
    if (status !== 'granted') return;

    const tokenData = await Notifications.getExpoPushTokenAsync();
    await removeDeviceToken(tokenData.data);
  } catch {
    // Silently ignore — device token removal is best-effort
  }
}
