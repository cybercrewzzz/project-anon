...
import { getBlockList, unblockUser } from '@/api/block';
...
const gradientColors = {
  user: ['#F6E0FF', '#F9FBFF', '#D2ECFE'] as const,
  volunteer: ['#CCF0FF', '#F9FBFF', '#D2ECFE'] as const,
};

export function BlockedUsersList({ role }: BlockedUsersListProps) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const [unblockTarget, setUnblockTarget] = useState<string | null>(null);

  const colors = gradientColors[role];
  ...
  const {
    ...
    queryKey: queryKeys.blocks(role),
  ...
  const unblockMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.blocks(role) });
    },
    onError: error => {
      setUnblockTarget(null);
